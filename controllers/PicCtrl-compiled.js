...
var getImageType = function getImageType(mimetype) {
    return mimetype.substring(6, mimetype.length);
};

var createPicImage = function createPicImage(image, name, contentType) {
...
module.exports = {
                        ...
                        croppedImage.toBuffer(imgType, function (err, buffer) {
                            ...
                            newPic.save(function (perr, picsResult) {
                                ...
                                newPicData.save(function (derr, dataResult) {
                                    if (derr) return res.status(500).send(derr);else res.send(dataResult);
                                });
                            });
                        });
                    });
    ...
    read: function read(req, res) {
        PicData.find(req.query).exec(function (err, result) {
            if (err) return res.status(500).send(err);else res.send(result);
        });
    },

    readFullPic: function readFullPic(req, res, next) {
    ...
    readThumbnail: function readThumbnail(req, res, next) {
        Pic.findById(req.query.id, function (err, doc) {
        ...
        PicData.findById(picDataId, function (err, doc) {
            ...
            Pic.findByIdAndRemove(picId, function (perr, result) {
                ...
                PicData.findByIdAndRemove(picDataId, function (derr, result) {
                    if (derr) return res.status(500).send(derr);else res.send(result);
                });
            });
        });
    }
...
//# sourceMappingURL=PicCtrl-compiled.js.map